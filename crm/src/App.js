...
class App extends Component {
  ...
  componentDidMount = async () => {
    let data = await axios.get('http://localhost:8080/allClients')
  ...
  updateClient = async (name, key, value) => {
    ...
    await axios.put('http://localhost:8080/updateClientFromActions', { name, key, value })
    await this.componentDidMount()
    return 'Updated successfully'
  }
  addClient = async (client) => {
    await axios.post('http://localhost:8080/addClient', client)
    await this.componentDidMount()
  }
  render() {
    return (
